    ...
    return (
        <div className="bg-white p-5 ">
            ...
            <div className="flex justify-between items-center gap-3 pb-3 border-b border-b-slate-300 mb-5">
                <h2 className="flex items-center gap-2 text-base md:text-lg font-bold"><FaMoneyCheckDollar className="mt-1" />Payment of Salary</h2>
            </div>

            {/* salary form */}
            <form onSubmit={handleAddSalary} className="space-y-2">
                {/* select employee */}
                <label htmlFor="select" className="font-bold">Select Employee</label>
                <select id="select" onChange={(e) => setSelectedEmployee(e.target.value)} className="w-full border border-slate-300 px-3 py-1 bg-white rounded-sm" defaultValue={'default'}>
